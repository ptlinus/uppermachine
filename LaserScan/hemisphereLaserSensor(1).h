...
class cubicStatisticMapper
{
	...
	float mwCell, mhCell, mMargin[4];
	float mXlen, mYlen, mZlen;
public:
	cubicStatisticMapper()	{ m_nn.nr = -1; xNum = yNum = total = 0; gcs = NULL; }
	~cubicStatisticMapper()	{}

	sensorLayout slayout;
	int xNum, yNum, total;	//网格数
	smGridCell *gcs;		//格子系列

	float xcubic()	{ return mXlen; }
	float ycubic()	{ return mYlen; }
	float zcubic()	{ return mZlen; }
	int numSensor()	{ return slayout.num; }

	int getCellID(int idSensor)	
	{
		sensorProfile *sp = slayout.get(idSensor);
		if (!sp)	return -1;
		return mapCellIndex(sp->fixedPosition[0], sp->fixedPosition[1]); 
	}

	bool load(char *configFile)
	{
		FILE *fp = NULL;
		if (!configFile || (fp = fopen(configFile, "r")) == NULL)
			return false;

		char title[80];
		fscanf(fp, "%s ", title);
		if (stricmp(title, "cubic") != 0)
			goto tagFailed;
		float xlen, ylen, zlen, wcell, hcell;
		fscanf(fp, "%f %f %f\n", &xlen, &ylen, &zlen);
		if (xlen <= 0 || ylen <= 0 || zlen <= 0)
			goto tagFailed;

		if (3 != fscanf(fp, "%s %f %f\n", title, &wcell, &hcell)
		|| stricmp(title, "grid") != 0	|| wcell <= 0 || hcell <= 0)
			goto tagFailed;

		if (!slayout.read(fp, xlen, ylen, zlen))
			goto tagFailed;

		fclose(fp);
		return create(wcell, hcell, xlen, ylen, zlen);
	tagFailed:
		fclose(fp);
		return false;
	}
	bool save(char *configFile)
	{
		FILE *fp = NULL;
		if (!configFile || (fp = fopen(configFile, "w")) == NULL)
			return false;

		fprintf(fp, "cubic %f %f %f\n", mXlen, mYlen, mZlen);
		fprintf(fp, "grid %f %f\n", mwCell, mhCell);
		slayout.write(fp);
	...
	bool create(float wGrid, float hGrid, float xLength, float yLength, float zLength)
	{
		gcs = NULL;
		...
		smGridCell *gc = gcs;
		float cellArea = mwCell * mhCell, yc = mhCell*0.5f, xc0 = mwCell*0.5f;
		...
		mMargin[0] = 0.05f*mwCell;			mMargin[1] = 0.05f*mhCell;
		mMargin[2] = mXlen - mMargin[0];	mMargin[3] = mYlen - mMargin[1];
		return true;
	}
	...
	void update(sphericalPack &pack)
	{
		if (!pack.valid())	return;
		sensorProfile *sp = slayout.get(pack.idSensor);
		if (!sp)	return;

		float x, y, z, zground = sp->fixedPosition[2] + 0.5f*mhCell;
		float xmargin = 0.1f*mwCell, ymargin = 0.1f*mhCell, xlimit = mXlen - xmargin, ylimit = mYlen - ymargin;
		for (int i = 0; i < pack.num; i++)
		{
			pack.sps[i].cartesian(x, y, z);

			x += sp->fixedPosition[0]; y += sp->fixedPosition[1];
			if (x < xmargin || x >= xlimit || y < ymargin || y >= ylimit || z <= 0 || z >= zground)
				continue;

			int id = ((int)(y / mhCell))*xNum + ((int)(x / mwCell));
			gcs[id].update(sp->fixedPosition[2] - z, pack.sps[i].intensity);
		}
	}
...
private:
	...
	{
		if (x < mMargin[0] || x > mMargin[2] || y < mMargin[1] || y > mMargin[3])	
			return -1;
		return ((int)(y/mhCell))*xNum + ((int)(x/mwCell));
	}
